...
public class SocialCircleInduction {
	...
	public static String[][] McAuleyInduction(EgoNetwork egoNetwork){
		...
		//Too inconsistent
		for(int a = 2; a < egoNetwork.numEdges(); a++){
	...
	public static String[][] McAuleyInduction(EgoNetwork egoNetwork, Writer writer) throws IOException{
		...
		numCircles = 10;
		return McAuleyInduction(egoNetwork, numCircles, writer);
		//return null;
		
	}
	
	public static String[][] McAuleyInduction(EgoNetwork egoNetwork, int numCircles, Writer writer) throws IOException{
		...
		for(int i = 0; i < currCircles.length; i++){
			...
			for(int j = 0; j < currCircles[i].length; j++)
				circles[i][j] = egoNetwork.getName(currCircles[i][j]);
			writer.write("Circle " + i + ": " + Arrays.toString(circles[i]));
			writer.write("Alpha " + i + ": " + alpha[i] + ", Theta " + i + Arrays.toString(theta[i]));
